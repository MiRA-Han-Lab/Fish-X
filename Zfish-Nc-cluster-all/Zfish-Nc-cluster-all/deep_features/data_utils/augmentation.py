...
def random_rotation(points, dev=180):
    points = rotation_3d_in_axis(points, [random.uniform(-1, 1) * dev], axis=0).squeeze()
    points = rotation_3d_in_axis(points, [random.uniform(-1, 1) * dev], axis=1).squeeze()
    points = rotation_3d_in_axis(points, [random.uniform(-1, 1) * dev], axis=2).squeeze()
    return points

def z_scaling(points, scale_range=(1.0, 1.5)):
    noise_scale = np.random.uniform(scale_range[0], scale_range[1])
    points[:, 2] *= noise_scale
    return points
...
def transform(points):
    ...
    # if random.random() < 0.50:
    #     augmentation_points = add_noisy(augmentation_points, noisy_nums=random.randint(20, 80))
    # if random.random() < 0.50:
    #     augmentation_points = z_scaling(augmentation_points)
    if random.random() < 0.75:
        augmentation_points = random_rotation(augmentation_points)
    return augmentation_points

# improve #2  obj sampling and Z scaling and small other aug.(missing parts)