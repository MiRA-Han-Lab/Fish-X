...
from connectomics.data.dataset.build import _get_file_list, _make_path_list
...
from ..data.mira_utils import cv_io
...
class Predictor(object):
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, sample in enumerate(self.dataloader):
                ...
                #imsave('img'+str(i).zfill(3)+'.tif',((volume[0,0].cpu().numpy()*0.5+0.5)*255).astype('uint8'))
                #imsave('msk'+str(i).zfill(3)+'.tif',(output[0,0:3].transpose([1,0,2,3])*255).astype('uint8'))
    ...
    def run_CV_PC(self, mode: str):
        ...
        #    executor.map(self.run_CV_box, bboxes)
    ...
    def run_CV_box(self, bbox):
        ...
        if not flag:
            # self.save_zeros([[z2 - z1, y2 - y1, x2 - x1]])
            return
    ...
    def write_task_progress(self,):
        from cloudvolume import CloudVolume
        from concurrent.futures import ProcessPoolExecutor
        chunk_begin = self.cfg.DATASET.VOLUME_START[::-1]
        chunk_end = self.cfg.DATASET.VOLUME_END[::-1]
        bboxes = chunk_bboxes(np.array(chunk_end) - np.array(chunk_begin), self.cfg.DATASET.DATA_CHUNK_SIZE[::-1],
                              offset=chunk_begin, mip=self.cfg.DATASET.MIP)
        vol = CloudVolume('file:///home/liuj/bigstore19/SCN/synapses/scn_all', mip=self.cfg.DATASET.MIP, parallel=1,
                          progress=False)

        def exists(coord):
            z1, z2, y1, y2, x1, x2 = coord
            coord_exist = vol.exists(np.s_[x1:x2, y1:y2, z1:z2])
        ...
        def process(bbox):
            ...
            x2, y2, z2 = bbox.max()
            coord = [z1, z2, y1, y2, x1, x2]
            coord_str = '-'.join(list(map(str, coord)))
            # if not os.path.exists(os.path.join(self.output_dir, self.test_filename)):
            if not exists(coord):
                # f.writelines(coord_str)
                return coord_str
            else:
                return ''
        # f.close()
        with ProcessPoolExecutor(max_workers=4) as executor:
            list_tasks = executor.map(process, bboxes[0:100])
            executor.shutdown()
            f.writelines('\n'.join(list(list_tasks)))
            f.close()
