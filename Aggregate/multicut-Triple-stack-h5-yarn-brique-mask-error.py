    ...
    try:
        ...
        #######################################################################################
           ...
           # boundaries_tests = boundaries_tests + raw_backgrounds
            ...
            print('error: ' + filelist[0])
            f = open('/opt/data/Nas402/zbrfish/liujz_code/' + '/' + 'error_2023.txt', mode='a')
            f.write('error:'+ '\t' + filelist[0] + '\n')  # write 写入
            f.close()  # 关闭文件
...
if __name__ == '__main__':
    ...
    if whether_Test == True:
        ...
        outPutList_stack = [] 
        for stack in range(468, 500):   #248, 336     #248, 303
        ...
        app_name = 'stack468-500'  # stack_name  # spark task name
        conf = SparkConf()
        conf.setMaster(spark_host)
        conf.setAppName(app_name)

        # task num = executors nums(spark.executor.instances) * core nums(spark.executor.cores)
        conf.set("spark.executor.instances", "12")  # executors nums; propsal:20 ~ 80
        conf.set("spark.executor.cores", "1")  # core num in each executor; propsal: 2 ~ 4
        conf.set("spark.default.parallelism", "12")
        # task memory
        conf.set("spark.driver.memory", "4g")  # driver memory; propsal :1G
        conf.set("spark.executor.memory", "80g")  # executor memory ; propsal: 4G~8G
        conf.set('spark.executor.heartbeatlnterval', '60s')  # communication waiting time

        sc = SparkContext(conf=conf)
        # sc.addPyFile("/opt/AlgorithmManager/RUNDATA.zip")
        # sc.addPyFile("/opt/AlgorithmManager/manager.zip")
        # sc.addPyFile("/opt/AlgorithmManager/algorithms.zip")

        # data list to rdd
        data = sc.parallelize(outPutList_stack)

        data = data.map(lambda x: test_multicut(x, pkl_load_path, zhezhou_mask_path, counter_file_path))
