...
parser.add_argument("--data_dir", type=str, default="../dataset/rgb/train")
parser.add_argument("--val_dirs", type=str, default="../dataset/rgb/validation")
parser.add_argument("--save_model_path", type=str, default="./experiments/results")
parser.add_argument("--log_name", type=str, default="vmapest_joint_unet_gauss25_12rf5")
...
def validation_cbsd68(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))
    fns.sort()
    images = []
    for fn in fns:
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, clean in enumerate(TrainingLoader):
        ...
        est_diff = (
            (est_val[:, 0:1] - est_val[:, 1:2]) ** 2
            + (est_val[:, 0:1] - est_val[:, 2:3]) ** 2
            + 2. * (est_val - 1.0)**2
        )
        ...
        # # 0.0 to opt.w_est 
        # w_est = (1-Lambda)*0.0+Lambda*opt.w_est 
        ...
        try:
            loss_brv = likelihood_loss(noisy, mu_m, sigma_m, sigma_n, train=True) # * (1+beta)/(1+Lambda2)
        except Exception:
            continue
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        valid_repeat_times = {"Kodak24": 1, "BSD300": 1, "Set14": 1, "CBSD68":1}
            ...
            for i in range(repeat_times):
                for idx, im in enumerate(valid_images):
                    ...
                    psnr_mu_m = calculate_psnr(
                        origin255.astype(np.float32), mu255_m.astype(np.float32)
                    )
                    ...
                    if i == 0:
                        ...
                        Image.fromarray(mu255_e).convert("RGB").save(save_path)
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_mu_m.png".format(valid_name, idx, epoch),
                        )
                        Image.fromarray(mu255_m).convert("RGB").save(save_path)
