...
parser.add_argument("--data_dir", type=str, default="../dataset/fmdd_sub/train")
...
parser.add_argument("--save_model_path", type=str, default="./experiments/vmap_est_fmdd")
parser.add_argument("--log_name", type=str, default="xxx_bbi_unet_fmdd_13rf21")
...
network = UNet(in_channels=opt.n_channel, out_channels=out_chn, wf=opt.n_feature)
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, noisy in enumerate(TrainingLoader):
        ...
        ## x or y
        # sigma_n = (1+beta**2)/(1+beta)**2*sigma_n
        ...
        for valid_name, valid_data in valid_dict.items():
            ...
            for idx in range(num_img):
                ...
                with torch.no_grad():
                    ...
                    dn_output = (
                        (network(net_input) * mask)
                        .view(n, -1, out_chn, h, w)
                        .sum(dim=1)
                    )
                    # exp
