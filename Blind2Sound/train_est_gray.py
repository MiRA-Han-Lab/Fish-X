...
parser.add_argument("--data_dir", type=str, default="../dataset/grayscale/train")
parser.add_argument("--val_dirs", type=str, default="../dataset/grayscale/validation")
parser.add_argument("--save_model_path", type=str, default="./experiments/results")
parser.add_argument("--log_name", type=str, default="est_pg30_25")
    ...
    def __init__(self, style):
        ...
        if style.startswith("gauss"):
            ...
            if '_' in style:
                ...
                self.div = False
                self.params = [float(p) for p in style.replace("pg", "").split("+")]
            if len(self.params) == 2:
                self.params[1] = self.params[1] / 255.0 if self.div else self.params[1]
                self.style = "pg_fix"
            elif len(self.params) == 4:
                self.params[2] = self.params[2] / 255.0 if self.div else self.params[2]
                self.params[3] = self.params[3] / 255.0 if self.div else self.params[3]
                self.style = "pg_range"
...
class DataLoader_GrayScale(Dataset):
    def __init__(self, data_dir, patch=128):
        ...
        self.data_dir = data_dir
        self.patch = patch
        self.train_fns = glob.glob(os.path.join(data_dir, '*.png'))        
        self.train_fns.sort()
        print('fetch {} samples for training'.format(len(self.train_fns)))
...
def validation_grayscale(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*.png"))
    fns.sort()
    images = []
    for fn in fns:
        im = Image.open(fn)
...
# Validation Set
BSD68_dir = os.path.join(opt.val_dirs, "BSD68")
Set12_dir = os.path.join(opt.val_dirs, "Set12")
Urban100_dir = os.path.join(opt.val_dirs, "Urban100")
valid_dict = {
    "BSD68": validation_grayscale(BSD68_dir),
    "Set12": validation_grayscale(Set12_dir),
    "Urban100": validation_grayscale(Urban100_dir)
}
# valid_dict = {
#     "BSD68": validation_grayscale(BSD68_dir)
# }
...
ratio = num_epoch / 100
optimizer = optim.Adam(estimator.parameters(), lr=opt.lr, weight_decay=opt.w_decay)
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, clean in enumerate(TrainingLoader):
        ...
        gat_noisy_sub_1 = torch.cat((
            gat_noisy[:,:,0:h//2,0:w//2], 
            gat_noisy[:,:,h//4:3*h//4,0:w//2],
            gat_noisy[:,:,h//2:h,0:w//2],
            gat_noisy[:,:,0:h//2,w//4:3*w//4], 
            gat_noisy[:,:,h//4:3*h//4,w//4:3*w//4],
            gat_noisy[:,:,h//2:h,w//4:3*w//4],
            gat_noisy[:,:,0:h//2,w//2:w], 
            gat_noisy[:,:,h//4:3*h//4,w//2:w],
            gat_noisy[:,:,h//2:h,w//2:w]), dim=1)
        ...
        est_diff_sub = torch.mean((est_val_sub - 1.0)**2, dim=1, keepdim=True)
        est_diff_sub_1 = torch.mean((est_val_sub_1 - 1.0)**2, dim=1, keepdim=True)
        est_diff = est_diff_sub #+ est_diff_sub + 0.5*est_diff_sub_1
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        np.random.seed(101)
        valid_repeat_times = {"BSD68": 1, "Set12": 1, "Urban100": 1}
