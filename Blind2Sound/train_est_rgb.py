...
# }
valid_dict = {"CBSD68": validation_kodak(CBSD68_dir)}
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, clean in enumerate(TrainingLoader):
        ...
        # print(est_val.shape, est_val_sub.shape)
        ...
        # print(est_val.shape, est_val_sub.shape)
        # est_diff_cramer = (
        #     (est_val[:, 0] - est_val[:, 1]) ** 2
        #     + (est_val[:, 0] - est_val[:, 2]) ** 2
        # )
        # est_diff = 2. * est_diff.repeat(1,4) + est_diff_sub + est_diff_cramer
