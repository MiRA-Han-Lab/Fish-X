...
parser.add_argument("--save_model_path", type=str, default="./experiments/est_fmdd")
parser.add_argument("--log_name", type=str, default="xxx_est_bbi_unet_fmdd")
...
class AugmentNoise(object):
    ...
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            print(
                "std={}, lam:{}".format(
                    std.squeeze().cpu().data.numpy(),
                    (1.0 / lam).squeeze().cpu().data.numpy(),
                )
            )
            # Poisson Gaussian Noise
            noised = torch.poisson(lam * x, generator=get_generator()) / lam
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for ii, noisy in enumerate(TrainingLoader):
        ...
        #         + torch.mean((est_val - 1.0)**2, dim=1)
        #     )
