...
parser.add_argument("--data_dir", type=str, default="../dataset/train/Imagenet_val")
...
parser.add_argument("--log_name", type=str, default="b2u_unet_gauss25_112rf20")
...
def get_SIDD_validation(dataset_dir):
    val_data_dict = loadmat(os.path.join(dataset_dir, "ValidationNoisyBlocksRaw.mat"))
    val_data_noisy = val_data_dict["ValidationNoisyBlocksRaw"]
    val_data_dict = loadmat(os.path.join(dataset_dir, "ValidationGtBlocksRaw.mat"))
...
# }
valid_dict = {"Kodak24": validation_kodak(Kodak_dir)}
...
# Network
network = UNet(
    in_channels=opt.n_channel, out_channels=opt.n_channel, wf=opt.n_feature
)
estimator = UNet(
    in_channels=opt.n_channel, out_channels=2, wf=opt.n_feature
)
if opt.parallel:
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    for iteration, clean in enumerate(TrainingLoader):
        ...
        print(f'noisy min: {torch.min(noisy)}, max: {torch.max(noisy)}')
        n, c, h, w = noisy.shape
        ...
        with torch.no_grad():
            ...
            est_map = torch.abs(est_map)
            sigma = torch.mean(est_map[:, 0:1, :, :], dim=[2, 3], keepdim=True)
            alpha = torch.mean(est_map[:, 1:2, :, :], dim=[2, 3], keepdim=True)
            gat_noisy = gat(noisy, sigma, alpha)
            print(f'gat_noisy min: {torch.min(gat_noisy)}, max: {torch.max(gat_noisy)}')
            gat_noisy, factor, z_min, z_max = norm_tensor(gat_noisy)
        # dn
        net_input, mask = masker.train(gat_noisy)
        dn_output = (network(net_input) * mask).view(n, -1, c, h, w).sum(dim=1)
        ##### add
        print(f'gat_dn min: {torch.min(dn_output)}, max: {torch.max(dn_output)}')
        #########
        dn_output = denorm_tensor(dn_output, z_min, z_max)
        print(f'denorm min: {torch.min(dn_output)}, max: {torch.max(dn_output)}')
        dn_output = inv_gat(dn_output, sigma, alpha, 'closed_form')
        print(f'invgat_dn min: {torch.min(dn_output)}, max: {torch.max(dn_output)}')
        print(f'sigma:{sigma.squeeze().data}, alpha: {alpha.squeeze().data}')
        diff = dn_output - noisy
        # exp
        with torch.no_grad():
            exp_output = network(gat_noisy)
            exp_diff = exp_output - gat_noisy
        ...
        optimizer.step()
        logger.info(
            "{:04d} {:05d} diff={:.6f}, exp_diff={:.6f}, Loss_Reg={:.6f}, Lambda={}, Loss_Rev={:.6f}, Loss_All={:.6f}, Time={:.4f}".format(
                epoch,
                iteration,
            ...
            for i in range(repeat_times):
                for idx, im in enumerate(valid_images):
                    ...
                    if epoch == opt.n_snapshot:
                        ...
                        noisy255 = np.clip(noisy255 * 255.0 + 0.5, 0, 255).astype(
                            np.uint8
                        )
                    # padding to square
                    H = noisy_im.shape[0]
                    ...
                    with torch.no_grad():
                        ...
                        sigma = torch.mean(est_map[:, 0:1, :, :], dim=[2, 3], keepdim=True)
                        alpha = torch.mean(est_map[:, 1:2, :, :], dim=[2, 3], keepdim=True)
                        gat_noisy = gat(noisy_im, sigma, alpha)
                        gat_noisy, factor, z_min, z_max = norm_tensor(gat_noisy)
                        # dn
                        net_input, mask = masker.train(gat_noisy)
                        dn_output = (network(net_input) * mask).view(n, -1, c, h, w).sum(dim=1)
                        ...
                        # exp
                        exp_output = network(gat_noisy)
                        exp_output = denorm_tensor(exp_output, z_min, z_max)
                        exp_output = inv_gat(exp_output, sigma, alpha, 'closed_form')
                    ...
                    pred255_exp = np.clip(pred_exp * 255.0 + 0.5, 0, 255).astype(
                        np.uint8
                    )
                    pred255_mid = np.clip(pred_mid * 255.0 + 0.5, 0, 255).astype(
                        np.uint8
                    )

                    # calculate psnr
                    psnr_dn = calculate_psnr(
                        origin255.astype(np.float32), pred255_dn.astype(np.float32)
                    )
                    avg_psnr_dn.append(psnr_dn)
                    ssim_dn = calculate_ssim(
                        origin255.astype(np.float32), pred255_dn.astype(np.float32)
                    )
                    avg_ssim_dn.append(ssim_dn)

                    psnr_exp = calculate_psnr(
                        origin255.astype(np.float32), pred255_exp.astype(np.float32)
                    )
                    avg_psnr_exp.append(psnr_exp)
                    ssim_exp = calculate_ssim(
                        origin255.astype(np.float32), pred255_exp.astype(np.float32)
                    )
                    avg_ssim_exp.append(ssim_exp)

                    psnr_mid = calculate_psnr(
                        origin255.astype(np.float32), pred255_mid.astype(np.float32)
                    )
                    avg_psnr_mid.append(psnr_mid)
                    ssim_mid = calculate_ssim(
                        origin255.astype(np.float32), pred255_mid.astype(np.float32)
                    )
                    ...
                    if i == 0:
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_dn.png".format(valid_name, idx, epoch),
                        )
                        Image.fromarray(pred255_dn).convert("RGB").save(save_path)
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_exp.png".format(valid_name, idx, epoch),
                        )
                        Image.fromarray(pred255_exp).convert("RGB").save(save_path)
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_mid.png".format(valid_name, idx, epoch),
                        )
