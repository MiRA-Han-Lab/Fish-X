...
parser.add_argument("--log_name", type=str, default="vmap_bbi_unet_gauss25_13rf21")
...
parser.add_argument("--increase_ratio", type=float, default=11.0)
...
optimizer = optim.Adam(
    [{"params": network.parameters()}, 
     {"params": estimator.parameters(), "lr":opt.lr*0.1}],
    lr=opt.lr,
            ...
            for i in range(repeat_times):
                for idx, im in enumerate(valid_images):
                    ...
                    with torch.no_grad():
                        ...
                        pme_e = likelihood_loss(
                            noisy_im, mu_e, sigma_e, sigma_n, train=False
                        )
                    ...
                    pme255_e = np.clip(pme_e * 255.0 + 0.5, 0, 255).astype(
                        np.uint8
                    )
                    ...
                    psnr_mu_e = calculate_psnr(
                        origin255.astype(np.float32), mu255_e.astype(np.float32)
                    )
                    ...
                    # pme_e
                    psnr_pme_e = calculate_psnr(
                        origin255.astype(np.float32), pme255_e.astype(np.float32)
                    )
                    avg_psnr_pme_e.append(psnr_pme_e)
                    ssim_pme_e = calculate_ssim(
                        origin255.astype(np.float32), pme255_e.astype(np.float32)
                    )
                    ...
                    if i == 0:
                        ...
                        save_path = os.path.join(
                            save_dir,
                            "{}_{:03d}-{:03d}_pme_e.png".format(valid_name, idx, epoch),
                        )
