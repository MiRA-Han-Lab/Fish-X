...
def make_label_z_level(max_block1_part_all, label, height, hang, line, direction):
...
def update_label(label, height, hang, line, list_old, list_new, direction, cuda_num = 0, width=15):
    ...
    if isinstance(width, int):
        ...
        ##########fastremap#########
        result_dict = dict(zip(list_old, list_new))

        #for num_height in range(0, height + 1):
        #    for num_hang in range(0, hang):
        #        for num_line in range(0, line):
        #            label[num_height][num_hang][num_line] = fastremap.remap(label[num_height][num_hang][num_line],
        #                                                                     result_dict, preserve_missing_labels=True,
        #                                                                     in_place=True)
        ############################
...
def merge_tif_bigdata_not_Regular_stack_merge_z_level_two(stack_path, save_path, raw_path, text_path, overloap_x_pixel, overloap_y_pixel, overloap_z_pixel, bite,
                                                cuda_num, width, start_list, end_list):
    ...
    for number, height in enumerate(end_list):
        if number >= len(end_list)-1 :
            print(number,' number >= len(end_list)-1')
            continue
        ...
        try:
            if not os.path.exists(save_path + '/' + all_stack[height-1] + '/' + all_stack[height-1] + '/' + "merge3.dat"):
                ...
                inital_z, inital_y, inital_x, label1 = load_binary_dat_break_down(save_path + '/' + all_stack[height - 1] + '/' + all_stack[height-1] + '/' + "merge3.dat")  
                print('load ', save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge3.dat", ' is successful')
        except:
        ...
        try:
            if not os.path.exists(save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat"):
                ...
                inital_z, inital_y, inital_x, label2 = load_binary_dat_break_down(save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat")  
                print('load ', save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat", ' is successful')
        ...
        row_new, row_next, label2 = make_label(block1_part, block2_part, max_block1_part_all, max_block2_part_all, label2,     
        ...
        ####
        row_new = zoom(row_new, [1, 0.5, 0.5], order = 0)
        row_next = zoom(row_next, [1, 0.5, 0.5], order = 0)
        print('zoom is over')
        ####
        ...
        ###
        if start_height < start_list[0]:
            start_height = start_list[0]
        ###
            ...
            if i == height:
                ...
                try:
                    if not os.path.exists(
                            save_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + "merge3.dat"):
                        inital_z, inital_y, inital_x, label = load_binary_dat_break_down(
                            save_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + "merge2.dat")  
                        print('in update! load ',
                              save_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + "merge2.dat", ' is successful')
                    else:
                        inital_z, inital_y, inital_x, label = load_binary_dat_break_down(
                            save_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + "merge3.dat")  
                        print('in update! load ',
                              save_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + "merge3.dat", ' is successful')

                except:
                    print(save_path + '/' + all_stack[i] + '/' + all_stack[i] + " merge2or3.dat is not existing!")
        ...
        for i in range(end_list[number] + 1, end_list[number+1]):
            ...
            label = make_label_z_level(max_block1_part_all, label, 0, hang = num_y_list[i], line = num_x_list[i], direction = 1)

            if i - end_list[number] <= width:
                start = time.process_time()
                label = update_label(label, 0, num_y_list[i], num_x_list[i], list_old, list_new, direction=0,
                                     cuda_num=cuda_num, width='all')  
                end = time.process_time()
                print(all_stack[i], ' update_label spend time is ', end - start)
            ...
            start = time.process_time()
            save_binary_dat(label, save_2_path, bite, 1, num_y_list[i], num_x_list[i], overloap_z_pixel,
                            overloap_y_pixel,
                            
                            overloap_x_pixel, SizeAray[i][num_y_list[i] - 1][num_x_list[i] - 1][0],
                            SizeAray[i][num_y_list[i] - 1][num_x_list[i] - 1][1],
                            SizeAray[i][num_y_list[i] - 1][num_x_list[i] - 1][2],
                            height, num_y_list[i] - 1, num_x_list[i] - 1,
                            save_name)  
    ...
    for height in range(0, total_z):
        ...
        try:
            if not os.path.exists(
                    save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat"):
                inital_z, inital_y, inital_x, label = load_binary_dat_break_down(
                    save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge2.dat")  
                print('in update! load ',
                      save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge2.dat", ' is successful')
            else:
                inital_z, inital_y, inital_x, label = load_binary_dat_break_down(
                    save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat")  
                print('in update! load ',
                      save_path + '/' + all_stack[height] + '/' + all_stack[height] + '/' + "merge3.dat", ' is successful')

        except:
            print(save_path + '/' + all_stack[height] + '/' + all_stack[height] + " merge2or3.dat is not existing!")
...
def make_big_label(raw_path, load_path, sorted_file, save_path, label, num_x, num_y, num_z, overloap_x_pixel,
    ...
    save_label_path = save_path + '/' + 'big_label2'
...
def overlay2(syn2D, i, color_map, raw):
    ...
    img2 = np.stack([raw[i], raw[i], raw[i]], axis=2)
    img2 = np.uint8(img2)
    raw_i = np.uint8(raw[i])
    
    
    img2 = img1 * 0.7 + img2 * 0.3

    whether = syn2D[i] == 0
    img2[:, :, 0][whether] = raw_i[whether]
    img2[:, :, 1][whether] = raw_i[whether]
    img2[:, :, 2][whether] = raw_i[whether]
    img2 = np.uint8(img2)
...
def write_big_label(stack_path, save_path, raw_path, overloap_x_pixel, overloap_y_pixel, overloap_z_pixel, start, end):
    ...
    for height in range(0, total_z):
        ...
        if not os.path.exists(save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge3.dat"):
            inital_z, inital_y, inital_x, label = load_binary_dat_break_down(save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge2.dat")  
            print('in writing! load ', save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge2.dat",
                  ' is successful')
        else:
            inital_z, inital_y, inital_x, label = load_binary_dat_break_down(save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge3.dat")  
            print('in writing! load  ', save_path + '/' + all_stack[height - 1] + '/' + all_stack[height - 1] + '/' + "merge3.dat",
                  ' is successful')
...
def delete_all_merge3():
    stack_path = '/home/liujz/bigstore3dian33_liujz/zbf_cellhe/zbf_cellhe_merge2/'
    stack_names = os.listdir(stack_path)
    for stack_name in stack_names:
        merge_path = stack_path + '/' + stack_name + '/' + stack_name + '/' + 'merge3.dat'
...
def align_big_png_all():
    label_path = '/home/liujz/liujz/2022_zbf_thire/zbf_cellhe_merge4/'
    save_path = '/home/liujz/liujz/2022_zbf_thire/zbf_cellhe_merge5/'
    text_path = '/home/liujz/liujz/2022_zbf_thire/zbf_all_stackresults_idx/'
    h5_path = '/home/liujz/liujz/2022_zbf_thire/zbf_cellhe_multicut_cc3d/'
    ...
    all_stack = os.listdir(label_path)
    all_stack.sort(key=lambda x: int(x[5:]))
    ...
    for i in range(len(all_stack)):
        ...
        sub_data_path = os.path.join(h5_path, all_stack[i])
        sub_tifs = os.listdir(sub_data_path)
        ...
        first_sorted_file = sub_tifs[0]
        size_z, size_y, size_x = get_one_block_size(h5_path + '/' + all_stack[i] +  '/' + first_sorted_file, whether_loadpath=False)
        overloap_y_pixel = 75
        overloap_x_pixel = 75
        


        text_path_1 = text_path + all_stack[i] + '/' + 'mosaic2fiji.txt'

        f = open(text_path_1)  
        
        line = f.readline()
        print(line)
        yoff_1 = np.int32(np.int32(line.split(',')[2])/8)
        xoff_1 = np.int32(np.int32(line.split(',')[1])/8)


        
        
        real_y = np.int32(first_sorted_file.split('_')[-2])
        suoyin_y =np.int32(line.split('.')[0].split('_')[-2])
        yoff_1 = yoff_1 + (real_y - suoyin_y) * (size_y - overloap_y_pixel)

        real_x = np.int32(first_sorted_file.split('_')[-1])
        suoyin_x = np.int32(line.split('.')[0].split('_')[-1])
        xoff_1 = xoff_1 + (real_x - suoyin_x) * (size_x - overloap_x_pixel)
        

        f.close()

        label_sub_path = label_path + '/' + all_stack[i] + '/' + all_stack[i]  + '/' + 'big_label4'
        all_png = os.listdir(label_sub_path)
        all_png.sort()


        for j, png_name in enumerate(all_png):
            number_z = number_z + 1
            print(number_z)

            result = np.zeros((12000,18000),dtype = np.int32)

            pngImage = imageio.imread(label_sub_path + '/' + png_name)
            a = pngImage.tobytes()
            b = np.frombuffer(a, dtype=np.uint32)
            c = b.reshape(pngImage.shape[0], pngImage.shape[1])
            

            result[yoff_1:yoff_1+pngImage.shape[0], xoff_1:xoff_1+pngImage.shape[1]] = c

            save_label_name = str(number_z).zfill(5)

            img2d = Image.fromarray(result, 'RGBA')
            img2d.save(os.path.join(save_path, save_label_name + '.png'))
...
def align_big_png_all_raw():
    label_path = '/home/liujz/bigstore3dian33_liujz/zbf_cellhe/zbf_cellhe_merge2/'
    save_path = '/home/liujz/bigstore3dian33_liujz/zbf_cellhe/zbf_cellhe_merge6_colorful/'
    text_path = '/home/liujz/bigstore3dian33_liujz/zbf-txt-idx/zbf_all_stackresults/'
    h5_path = '/home/liujz/bigstore3dian33_liujz/zbf_cellhe/zbf_cellhe_multicut_cc3d/'
    ...
    for i in range(len(all_stack)): 
        ...
        first_sorted_file = sub_tifs[0]
        size_z, size_y, size_x = get_one_block_size(h5_path + '/' + all_stack[i] + '/' + first_sorted_file,
                                                    whether_loadpath=False)
        overloap_y_pixel = 75
        overloap_x_pixel = 75
        ...
        label_sub_path = label_path + '/' + all_stack[i] + '/' + all_stack[i] + '/' + 'colorful'  
        all_png = os.listdir(label_sub_path)
        all_png.sort()
        ...
        for j, png_name in enumerate(all_png):
            ...
            result = np.zeros((12000, 18000, 3), dtype=np.uint8)
            ...
            result[yoff_1:yoff_1 + pngImage.shape[0], xoff_1:xoff_1 + pngImage.shape[1], :] = pngImage
            ...
            imageio.imwrite(save_path + '/' + save_label_name + '.tif', result)
...
if __name__ == '__main__':
    ...
    #start_list = [0,50,100,150,200,250,300,350,400,450,500,550,600,650,700]
    #end_list = [50,100,150,200,250,300,350,400,450,500,550,600,650,700,763]
    #start_list = [0,50,100,150,200,222,245,250,266,284,285,296,300,332,350,375,399,400,425,450,480,500,550,600,650,700]
    #end_list = [50,100,150,200,222,245,250,266,284,285,296,300,332,350,375,399,400,425,450,480,500,550,600,650,700,763]
    
    
    #start_list = [0, 50,100,150,200,222,245,250,266,   284,285,296,300,332,   350,375,399,400,   425,450,480,500,   550,600,650,700]
    #end_list =  [50,100,150,200,222,245,250,266,284,   285,296,300,332,350,   375,399,400,425,   450,480,500,550,   600,650,700,763]
    
    start_list = [0, 284, 285, 350, 425, 550]
    end_list =  [284, 285, 350, 425, 550, 763]
    
    
    merge_tif_bigdata_not_Regular_stack_merge_z_level_two(stack_path, save_path, raw_path, text_path, overloap_x_pixel, overloap_y_pixel, overloap_z_pixel, bite, cuda_num,
                                        width, start_list, end_list)    
    #load_binary_dat_save_tif('/home/liujz/liujz/2022_zbf_thire/zbfStackResults_merge/stack540/merge.dat', '/home/liujz/liujz/2022_zbf_thire/zbfStackResults_third_multicut/stack540/', '/home/liujz/liujz/2022_zbf_thire/zbfStackResults_merge/stack540/stack540')
    ...
    #write_big_label(stack_path3, save_path3, raw_path, overloap_x_pixel, overloap_y_pixel, overloap_z_pixel, start, end)

    

    #zbf_cellhe_processed_all(stack_path, save_path2, start, end)
